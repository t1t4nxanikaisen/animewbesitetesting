      ...
      <Link to={`/watch/${safeId}`} className="block w-full">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex flex-col gap-1 items-end">
            {episode.subStreams?.length > 0 && <span className="text-xs px-2 py-1 rounded bg-neutral-800">SUB</span>}
            {episode.dubStreams?.length > 0 && <span className="text-xs px-2 py-1 rounded bg-neutral-800">DUB</span>}
            {episode.hindiStreams?.length > 0 && <span className="text-xs px-2 py-1 rounded bg-yellow-400 text-black">HINDI</span>}
          </div>
        </div>
      </Link>
    </li>
